...
def strip_n_fill(df,outliers=False,add_elev=False):
...
def read_parquet_batch(cwd, fdir, file_list, seq_length, scaleX=None, scaleT=None, debug=False,outliers=False,add_elev=False):
    ...
    for file in file_list:
        ...
        Xdf, Tdf = strip_n_fill(df,outliers,add_elev)
...
def read_parquet_flight_merge(cwd, fdir, file_list, seq_length, scaleX=None, scaleT=None, outliers=False,add_elev=False):
    ...
    for file in file_list:
        ...
        Xdfnew, Tdfnew = strip_n_fill(df,outliers,add_elev)
...
def add_elevation(Xdf,file=None):
    ...
    with open(coord_file, 'w') as f:
        for i in range(n_samples):
            f.write(f"{Xdf['LONP'][i]}  {Xdf['LATP'][i]}")
            f.write('\n')
